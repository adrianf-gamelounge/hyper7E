...
1. If the invocation gives an absolute path anywhere under `.hyper/` (e.g. `.hyper/tasks/`, `.hyper/archive/`, `.hyper/backlog.md`, `.hyper/team/`), derive the Hyper state root from that path first (strip the `.hyper/` component and its descendants).
2. Otherwise, if the current directory is not inside a Git repository, use the current working directory as the Hyper state root.
...
5. If worktree discovery fails or only bare entries exist, fall back to `git rev-parse --show-toplevel`; if that fails too, use the current working directory as the Hyper state root.
