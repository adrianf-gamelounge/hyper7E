---
name: hyper-explore
description: Runs the explore phase of a Hyper task. Clarifies the user's goal, scans the codebase for relevant files and patterns, classifies scope (quick, feature, or research), and writes an approved exploration.md with findings and a proposed approach. Use when a Hyper task is in the 'explore' phase (check task.md frontmatter), or when the user wants to investigate a problem before implementing. Dispatched by the hyper skill, can also be invoked directly. Keywords: hyper, explore, investigate, clarify, approach, scope, exploration.md.
---
